...
const Wishlist = () => {
  ...
  if (!userId) return <p className="text-center text-red-500">User not found or not logged in.</p>;
          ...
          {wishlist.map((item) => (
            ...
            >
              ...
              <div className="flex-1 sm:ml-4 text-sm text-gray-800">
                ...
                <p className="mt-1"><strong>Description:</strong> {item.product.description}</p>
                <p><strong>Category:</strong> {item.product.category}</p>
                <p><strong>Price:</strong> <span className="font-semibold text-orange-600">{item.product.price} €</span></p>
                <p className="text-xs text-gray-400 mt-1">
                  Added on: {new Date(item.createdAt).toLocaleDateString()}
                </p>
                <p className="text-xs text-gray-400 mt-1">Shop: SmartBuy - The first online shopping center in Kosovo</p>
              </div>
              {/* Buttons */}
              <div className="flex flex-col gap-2 mt-4 sm:mt-0">
                ...
                >
                  <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="orange" strokeWidth="2">
                ...
                >
                  <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="red" strokeWidth="2">
                    <path d="M3 6h18" />
